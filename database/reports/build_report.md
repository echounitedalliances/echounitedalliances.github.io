...
Generated 2026-09-23 21:20 UTC by `database/scripts/build_database.py`.
...
| aircraft | 194,693 |
| flights | 358,244 |
| flight_assignments | 445,957 |
| airline_hubs | 3,449 |
| airline_stats | 58 |

## Data conditions handled

- 6,771 flights depart outside the 0-86399s day and were split into a time of day plus a signed day offset; the raw value is kept in `departure_daily_seconds_raw`.
- 64 aircraft have cabin ratios that do not sum to 1.0; they are loaded as exported and flagged by `v_aircraft_ratio_anomalies`.
...
- 594 airlines had no hubAirports in their roster; their hubs were derived from the base airports of their fleet

### identity

- 518 airlines kept their carrier_code, 90 were assigned one; 413 of 608 carry a division-qualified code because the game code is shared or was once
- proxima/unknown_81846bbb: airline has no name in the export (uid 81846bbb-d6db-4c13-a507-7a1fd8df0cf8)

### livery

- 547 of 608 liveries yield a brand colour; the rest fly white and fall back to the division accent
...
- 32 (airline, flight number, origin, destination) combinations appear on more than one flight_id - players may file the same number twice, so that tuple is indexed but not unique
- all 12155 stopover children resolve to a known flight

### stats

- member stats exist for 58 of 608 airlines (only Aegis exported members_stats.json)
